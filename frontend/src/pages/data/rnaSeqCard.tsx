...
const RnaSeqCard = () => {
  ...
  const {
    ...
    const selectedDiseaseData = filters.disease.includes("All")
      ? dataArray
      : dataArray.filter((data) =>
          filters.disease.some(
    ...
    const microarrayCountInHuman = selectedDiseaseData.filter(
      (data) =>
        data.StudyType === "Microarray" && data.Organism?.[0] === "Human"
    ).length;
  ...
  const handleFilterChange = useCallback((key, value) => {
    if (key === "disease") {
      if (value.includes("All")) {
        ...
        filters.disease.length === indications.length &&
  ...
  useEffect(() => {
    if (filteredData && filters?.disease) {
      ...
      register("rnaseq", {
        disease: filters.disease.includes("All")
    ...
    <article id="rnaSeq">
      ...
      {filters.disease && !isLoading && dataArray.length > 0 && (
        <section className="mb-10">
          ...
          <span>
            {" "}
            <span className="text-sky-800">{totalStudies} </span>studies for
            selected disease:{" "}
            <span className="text-sky-800">{bulkRnaCount}</span> bulk-RNA{" "}
            {" ( "}
            <span className="text-sky-800">{bulkRnaCountInHuman}</span> for
            Human {" ), "}
            <span className="text-sky-800">{singleRnaCount}</span> sc-RNA{" "}
            {" ( "}
            <span className="text-sky-800">{singleRnaCountInHuman}</span> for
            Human {" ), "}
            <span className="text-sky-800">
              {MicroarrayCount}
            </span> microarrays {" ( "}
            <span className="text-sky-800">{microarrayCountInHuman}</span> for
            Human {" ) "}
          </span>
